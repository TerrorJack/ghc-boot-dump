...
{-# LINE 17 "GHC/IO/Handle/Lock.hsc" #-}
...
{-# LINE 56 "GHC/IO/Handle/Lock.hsc" #-}
...
{-# LINE 178 "GHC/IO/Handle/Lock.hsc" #-}

lockImpl :: Handle -> String -> LockMode -> Bool -> IO Bool
lockImpl h ctx mode block = do
  FD{fdFD = fd} <- handleToFd h
  let flags = cmode .|. (if block then 0 else 4)
{-# LINE 183 "GHC/IO/Handle/Lock.hsc" #-}
  ...
  where
    cmode = case mode of
      SharedLock    -> 1
{-# LINE 193 "GHC/IO/Handle/Lock.hsc" #-}
      ExclusiveLock -> 2
{-# LINE 194 "GHC/IO/Handle/Lock.hsc" #-}

unlockImpl :: Handle -> IO ()
unlockImpl h = do
  FD{fdFD = fd} <- handleToFd h
  throwErrnoIfMinus1_ "flock" $ c_flock fd 8
{-# LINE 199 "GHC/IO/Handle/Lock.hsc" #-}

foreign import ccall interruptible "flock"
  c_flock :: CInt -> CInt -> IO CInt


{-# LINE 265 "GHC/IO/Handle/Lock.hsc" #-}
